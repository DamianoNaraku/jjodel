...
export class Type {
    ...
    static makeTypeSelector(selectHtml, selectedType, addPrimitive, addEnum, addClass, addVoid) {
        ...
        U.clear(selectHtml);
        const mp = ModelPiece.getLogic(selectHtml);
        ...
        // Enum Start:
        if (addEnum) {
            const enumarr = Status.status.mm.getAllEnums();
        ...
        // class Start:
        if (addClass) {
            const classarr = Status.status.mm.getAllClasses();
    ...
    applyTypeStr0() {
        ...
        if (!this.primitiveType) {
            ...
            const packages = Status.status.mm.childrens;
