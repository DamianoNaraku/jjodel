...
Status.status = null;
export function main0(loadEvent, tentativi = 0) {
...
function main() {
    ...
    for (j = 0; j < vertexposMat.length; j++) {
        ...
        for (let key in vdic) {
            ...
            const size = new GraphSize().clone(vdic[key]);
            if (!mp || !(mp instanceof IClassifier)) {
                U.cclear();
                U.pw(true, 'invalid vertexposition save, failed to get classifier:', key, vdic);
                continue;
            }
    ...
    IEdge.all.forEach((e) => { e.refreshGui(); });
